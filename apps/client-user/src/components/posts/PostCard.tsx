...
export function PostCard({ post, currentUserId, onDelete, isFocused }: PostCardProps) {
	...
	let cw = null;
	const cwMatch = content.match(/^<CW>(.*?)<\/CW>\n?(.*)$/s);
	if (cwMatch) {
		cw = cwMatch[1];
		content = cwMatch[2];
	}
	...
	const authorDisplayName = post.author?.displayName || "Deleted User";
	const authorUsername = post.author?.username || "deleted";
	...
	return (
		<article {...stylex.props(styles.article)} data-post-id={post.id}>
			{/* Revaye attribution banner */}
			{post.repostedBy && (
				<div {...stylex.props(styles.revayeBanner)}>
					<Repeat2 size={13} />
					<Link
						to="/users/$username"
						params={{ username: post.repostedBy.username }}
						{...stylex.props(styles.revayeBannerLink)}
						data-testid="revaye-attribution"
					>
			...
			<div {...stylex.props(styles.container)}>
				<Link to="/users/$username" params={{ username: authorUsername }}>
					<UserAvatar avatarUrl={authorAvatar} username={authorUsername} />
				</Link>
				...
				<div {...stylex.props(styles.content)}>
					...
					{/* Content Warning Box */}
					{cw && !cwRevealed ? (
						<div {...stylex.props(styles.cwBox)} data-testid="cw-display">
							<span {...stylex.props(styles.cwText)}>
					...
					<div {...stylex.props(styles.actions)}>
						...
						<RevayeButton
							postId={post.id}
							isOwnPost={isOwnPost}
							initialRevayeed={post.isRevayeed ?? false}
							initialCount={post.revayeCount ?? 0}
						/>
