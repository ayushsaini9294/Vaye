...
import { getUserRevayes } from "../../server/functions/revayes";
		...
		try {
			const [profileUser, userPosts, revayeedPosts, currentU, followers, following] =
				await Promise.all([
					...
					getUserRevayes({ data: username }),
					getCurrentUser(),
					getFollowerCount({ data: username }),
					getFollowingCount({ data: username }),
				]);

			// Merge own posts and revayeed posts, sort newest first
			const combined = [...userPosts, ...revayeedPosts].sort(
