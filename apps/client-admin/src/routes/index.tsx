...
import { getDashboardStatsFn } from "../server/functions/admin";

export const Route = createFileRoute("/")({
	beforeLoad: requireAdminAccess,
	loader: async () => {
		const statsData = await getDashboardStatsFn();
		return { statsData };
	},
	...
	const stats = [
		{
			...
			change: `+${statsData.newUsersToday} today`,
			positive: true,
			icon: Users,
			color: "blue",
		},
		{
			...
			change: `+${statsData.newPostsToday} today`,
			positive: true,
			icon: FileText,
			color: "green",
		},
		...
		},
		{
			...
			change: statsData.pendingReports === 0 ? "All clear" : "Action needed",
	...
	return (
		<main {...stylex.props(styles.container)}>
			<header {...stylex.props(styles.header)}>
				...
				<p {...stylex.props(styles.subtitle)}>Overview of Vaye platform activity</p>
			</header>
